...
#from config import *
#from yoloNN import blockGenerator, yoloReshape
...
def YOLOv3_tiny(input_shape, no_class, no_anchors):
    ...
    out1, out2, inputs = darknet19(input_shape)
...
def Create_Yolov3(input_size=YOLO_INPUT_SIZE, channels=YOLO_CHANNELS,
    ...
    inputs, smallBNB, largeBNB = YOLOv3_tiny(input_layer, no_classes, no_anchors)
    conv_tensors = [smallBNB, largeBNB]
    decoder1 = yoloReshape(input_size, conv_tensors[0].shape[2], anchors[0], no_classes)
    decoder2 = yoloReshape(input_size, conv_tensors[1].shape[2], anchors[1], no_classes)
    decoders = [decoder1, decoder2]
    output_tensors = []

    for i, (decoder, conv_tensor) in enumerate(zip(decoders, conv_tensors)):
        pred_tensor = decoder(conv_tensor)
        #if training: output_tensors.append(conv_tensor)
        output_tensors.append(pred_tensor)

    YoloV3 = Model(inputs, output_tensors)
    return YoloV3
'''
#Pruebas
# A,B = YOLOv3_tiny((YOLO_INPUT_SIZE,YOLO_INPUT_SIZE,1),10,tf.shape(ANCHORS)[1])
# print(A.shape)
# print(B.shape)
model = Create_Yolov3(input_size=YOLO_INPUT_SIZE, channels=YOLO_CHANNELS,
